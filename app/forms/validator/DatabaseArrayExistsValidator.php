...
class DatabaseArrayExistsValidator extends Validator
{
    /**
    ...
    public function validate(Validation $validator, $attribute)
    {
        ...
        $fieldsValues = $this->validator->getValue($this->attribute);

        if (!is_array($fieldsValues) || count($fieldsValues) < 1) {
            $this->appendMessage();
            return false;
        }

        foreach ($fieldsValues as $fieldsValue) {
            if (!$this->registerExists($fieldsValue)) {
                $this->appendMessage();
                return false;
            }
        }
